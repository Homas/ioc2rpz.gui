...
RUN unzip /tmp/fontawesome-free-5.9.0-web.zip -d /tmp && cp /tmp/fontawesome-free-5.9.0-web/css/all.css /opt/ioc2rpz/www/css && cp -r /tmp/fontawesome-free-5.9.0-web/webfonts /opt/ioc2rpz/www/ && sed -i -e "s#^.*Docker_CSS.*#${Docker_CSS}#" -e "s#^.*Docker_JS.*#${Docker_JS}#" -e "s/^.*\(<!-- Docker_Comm_Start\).*/\1/" -e "s/^.*Docker_Comm_End.*/-->/" /opt/ioc2rpz/www/index.php && sed -i -e "s#^.*Docker_CSS.*#${Docker_CSS}#" -e "s#^.*Docker_JS.*#${Docker_JS}#" -e "s/^.*\(<!-- Docker_Comm_Start\).*/\1/" -e "s/^.*Docker_Comm_End.*/-->/" /opt/ioc2rpz/www/io2auth.php && chmod 644 /opt/ioc2rpz/www/css/* && chmod 644 /opt/ioc2rpz/www/js/* && chmod 644 /opt/ioc2rpz/www/webfonts/* && rm -rf /tmp/*
