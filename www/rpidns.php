...
<div>
	<b-card header-class="bold" style="max-height:calc(100vh - 100px)">
      ...
      <b-row>
		...
		<b-container ref="RpiDNSCards" fluid class="pt-2 pb-2 ps-2" style="overflow-y:scroll;position:relative;height: calc(80vh - 100px)">
		<template v-for="(cardrow, cardrowIndex) in RpiDNSListDash" :key="cardrowIndex">			
			<b-card-group deck>
...
<!-- modal add RpiDNS --> 
	 <b-modal centered size="lg" :title="RpiDNSLabel" id="mAddRpiDNS" v-model="modalVisibility.mAddRpiDNS" ref="refAddRpiDNS" body-class="pt-0 pb-0" :ok-title="RpiDNSBttn" @ok="add_rpidns" v-cloak>
		 <span class=''>
			<b-container fluid>
				<b-row class="pb-2">
					<b-col md="12" class="p-0">
						<b-form-input v-model.trim="addRpiDNSName" ref="refAddRpiDNSName" :state="validateHostnameOnly('addRpiDNSName')" :formatter="formatName" ref="formAddRpiDNS" placeholder="Enter name"  v-b-tooltip.hover title="Name" />
					</b-col>
				</b-row>
						...
						<template>
								<div >
									<b-table striped hover sticky-header="300px" no-border-collapse :provider="createTableProvider('/io2data.php/rpzs')" :fields="tRPZRpiDNS_fields" ref="tRPZRpiDNS" small>
										...
										<template #cell(action)="row" :id="'rpidns_action_'+row.item.name">
											<b-form-select size="sm"  v-model="ftRpiDNSRPZAction[row.item.name]">
												<option v-for="action in addRpiDNSFeedActionComp(row.item.action)" :key="action.value" :value="action.value">{{action.text}}</option>
											</b-form-select>
										</template>
				...
				<b-row class="pb-1">
					<b-col md="12" class="p-0">
						<b-form-textarea rows="2" max-rows="3" maxlength="250" v-model.trim="addRpiDNSComment" ref="formAddRpiDNSComment" placeholder="Commentary"  v-b-tooltip.hover title="Commentary" />						
					</b-col>
				</b-row>
